...
const PaymentFormHook = ({ userDataPay, handleReturnForm, isIndividualPay }: { userDataPay: any, handleReturnForm: () => void, isIndividualPay: boolean }) => {
    ...
    const totalAmountToPay = isIndividualPay
        ...
        : userDataPay.reduce((total: any, item: any) => total + (item.userOrder?.totalAmount || 0), 0);
    ...
    const createTransaction = async (transactionBody: any) => {
        const authToken = paymentSource ? `Bearer ${wompiConfig.WOMPI_PRIVATE_KEY}` : `Bearer ${wompiConfig.WOMPI_PUBLIC_KEY}`;
    ...
    const handlePayment = async (e: any) => {
        ...
        try {
            ...
            let transactionBody;
            if (paymentSource) {//Usar una tarjeta existente
                transactionBody = {
