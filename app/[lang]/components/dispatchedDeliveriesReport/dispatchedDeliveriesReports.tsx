...
import { Box, Modal, IconButton, InputAdornment, Tooltip } from "@mui/material";
...
const PendingPaymentReports = ({ handlePayUser }: { handlePayUser: any }) => {
  ...
  const calculateTotalDiscount = () => {
    ...
    // Calculando el total de productos sin multiplicar por las cantidades
    const productsPrice =
      detalleCompra?.userOrder?.selectedProducts?.reduce(
        (acc: number, product: any) =>
          acc +
          (product.categoryPrice || 0) + // Sin multiplicar por cantidad
          (product?.full_price_Discount || 0), // Sin multiplicar por cantidad
        0
      ) || 0;
